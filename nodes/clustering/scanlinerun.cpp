...
#include <points_preprocessor_usi/segConfig.h>
...
public:
    ...
    double max_distance_y;// threshold of distance of runs to be merged.
    double cluster_th;// threshold of distance of runs to be merged.
private:
    ...
    int interest_line_= 8;
    int interest_idx_ = 500;
#endif
};
...
ScanLineRun::ScanLineRun():node_handle_("~"){
    ...
    laser_row_ = std::vector<SLRPointXYZIRL>(5340, p_dummy);
    // laser_frame_ = std::vector<std::vector<SLRPointXYZIRL> >(32, laser_row_);
    laser_frame_ = std::vector<std::vector<SLRPointXYZIRL> >(8, laser_row_);
    ...
    node_handle_.param("th_run", th_run_, 0.05);
    ...
    node_handle_.param("line", interest_line_, 0);
...
void ScanLineRun::update_labels_(int scan_line){
    ...
    for(int j_idx=0;j_idx<point_size_j_idx;j_idx++){
        ...
        for(int l=scan_line-1;l>=0;l--){
            ...
            int nn_idx = j;
            // int nn_idx = smart_idx_(j, ng_idx_[l].size(), ng_idx_[scan_line].size(), false);
...
visualization_msgs::Marker mark_cluster(pcl::PointCloud<SLRPointXYZIRL>::Ptr cloud_cluster,
  ...
  marker.header.frame_id = "top_laser_link";
...
void ScanLineRun::velodyne_callback_(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg){
    ...
    SLRPointXYZIRL p_dummy;
    p_dummy.intensity = -1;
    laser_row_ = std::vector<SLRPointXYZIRL> (5400, p_dummy);
    laser_frame_ = std::vector<std::vector<SLRPointXYZIRL> >(8, laser_row_);
    ...
    for(auto &point:laserCloudIn.points){
        if(point.ring<sensor_model_&&point.ring>=0){
            ...
            if (isnan(point.x))
                continue;
            if (point.y > max_distance_y || point.y < -max_distance_y)
              continue;
            range = sqrt(point.x*point.x + point.y*point.y + point.z*point.z);
            if(point.y>=0){
                row = int(1334 - asin(point.x/range)/0.001176626462);
            }else if(point.y<0 && point.x <=0){
                row = int(4004 + asin(point.x/range)/0.001176626462);
            }else {
                row = int(4004 + asin(point.x/range)/0.001176626462);
            }
...
// #endif
          ...
          if (clusters->points.size() > 15) {
            num_clusters++;
            visualization_msgs::Marker mk = mark_cluster(clusters, "p", cnt, 0, 0, 250);
            if (mk.scale.y  < 3.0)
...
// #endif
    ...
    ROS_INFO("Total cluster: %d", num_clusters);
    ...
    vector<visualization_msgs::Marker >::iterator it = ma.markers.begin();

    while(it != ma.markers.end()) {

        vector<visualization_msgs::Marker >::iterator it2 = it + 1;
        while (it != ma.markers.end() && it2 != ma.markers.end()) {
          if(dist(it->pose.position, it2->pose.position) < cluster_th) {
            it->color.r = 250;
            it->color.g = 0;
            it->color.b = 0;
            it->color.a = 0.4;

              it2 = ma.markers.erase(it2);
          } else {
            it2++;
          }
        }
...
void callback(points_preprocessor_usi::segConfig &config, uint32_t level) {
  ptr->th_run_ = config.th_run;
  ptr->th_merge_ = config.th_merge;
  ptr->max_distance_y = config.max_distance_y;
  ptr->cluster_th = config.cluster_th;
...
int main(int argc, char **argv)
{
    ...
    dynamic_reconfigure::Server<points_preprocessor_usi::segConfig> server;
    dynamic_reconfigure::Server<points_preprocessor_usi::segConfig>::CallbackType f;
